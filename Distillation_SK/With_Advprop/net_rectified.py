...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.fc = nn.Linear(512 * block.expansion, num_classes)  # global average pooling in other resnet
        # self.linear = nn.Conv2d(512, 10, kernel_size=1)
    ...
    def _forward_impl(self, x, is_feat=False, preact=False):
        # See note [TorchScript super()]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)

        # x = self.maxpool(x)# additional part of RESNET
        ...
        if is_feat:
            if preact:
                activation1 = self.layer2[0].bn1(activation1)
                activation2 = self.layer3[0].bn1(activation2)
                activation3 = self.layer4[0].bn1(activation3)
            return [activation0, activation1, activation2, activation3, activation4], x
        else:
            return x
...
class AdvResNet(ResNet):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, attacker=NoOpAttacker()):  # num_class = 1000
        ...
        self.mixbn = False  # default False
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        input_len = len(x)
        images = x
        return self._forward_impl(images, is_feat, preact)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load("/home/yaof/apex/deep_dream/cache/models/WRN50-2teacher_b")
