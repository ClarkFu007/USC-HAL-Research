...
# Models
default_model_names = sorted(name for name in net_rectified.__dict__
                             if name.islower() and not name.startswith('__')
                             and callable(net_rectified.__dict__[name])
                             and not name.startswith("to_") and not name.startswith("partial"))
...
# Datasets
parser.add_argument('--data', default='/home/yaof/apex/deep_dream/cache/data/', type=str,
                    help='path to dataset')
parser.add_argument('--workers', default=4, type=int, metavar='N',
                    help='number of data loading workers (default: 4)')  # my computer cannot handle the multistread
...
# Checkpoints
parser.add_argument('-c', '--checkpoint', default='/home/yaof/AdvProp/saved_models',
...
# /home/yaof/AdvProp/saved_models/checkpoint.pth.tar
parser.add_argument('--resume', default='',
                    type=str, metavar='PATH', help='path to latest checkpoint.')  # Continue training
# /home/yaof/AdvProp/saved_models/model_best.pth.tar
parser.add_argument('--load', default='',
                    type=str)  # Just evaluation

# Architecture
parser.add_argument('--arch', '-a', metavar='ARCH', default='wide_resnet50_2', choices=model_names,
                    help='model architecture: ' + ' | '.join(model_names) + ' (default: resnet18)')
# Miscs
parser.add_argument('--manualSeed', type=int, help='manual seed')
parser.add_argument('-e', '--evaluate', default=False, dest='evaluate', action='store_true',
                    help='evaluate model on validation set')  # Just evaluation
...
parser.add_argument('--mixbn', action='store_true', help='use mixbn', default=True)     # Use mix or not
...
parser.add_argument('--attack-iter', help='Adversarial attack iteration', type=int, default=5)
parser.add_argument('--attack-epsilon', help='Adversarial attack maximal perturbation', type=float, default=4.0)
...
print("The satement that we are running on the GPU is", use_cuda)
...
def main():
    ...
    train_dataset = datasets.CIFAR10(root=args.data, train=True, download=False, transform=transform_train)
    ...
    val_dataset = datasets.CIFAR10(root=args.data, train=False, download=False,
    ...
    if args.mixbn:
        ...
        norm_layer = None
    model = net_rectified.__dict__[args.arch](num_classes=args.num_classes, norm_layer=norm_layer)
    ...
    if args.resume:
        ...
        if args.load:
            # Load checkpoint.
            print('==> Evaluating fron the best model..')
            ...
            model.load_state_dict(checkpoint['state_dict'], strict=False)  # "Strict" can handle DataParallel problem
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_checkpoint({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'acc': test_acc,
                         'best_acc': best_acc, 'optimizer': optimizer.state_dict()},
                        is_best, checkpoint=args.checkpoint)

    print('Best acc:')
    print(best_acc)
    # torch.save(model.state_dict(), args.checkpoint + 'WRN50-2 teacher')
    # writer.close()
    # logger.close()
    # logger.plot()
...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda, warmup_scheduler, mixbn=False,
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        """
        bar.suffix = '({batch}/{size}) Data: {data:.2f}s | Batch: {bt:.2f}s | ' \
                     'Total: {total:} | ETA: {eta:} | Loss: {loss:s} | top1: {top1:s} | top5: {top5: .1f}'\
            .format(batch=batch_idx + 1, size=len(train_loader), data=data_time.val, bt=batch_time.val,
                    total=bar.elapsed_td, eta=bar.eta_td, loss=loss_str, top1=top1_str, top5=top5.avg)
        bar.next()
        
        # total=bar.elapsed_td, eta=bar.eta_td
        print('({batch}/{size}) Data: {data:.2f}s | Batch: {bt:.2f}s | '
              'Total: {total:} | ETA: {eta:} | Loss: {loss:s} | top1: {top1:s} | top5: {top5: .1f}'
              .format(batch=batch_idx + 1, size=len(train_loader), data=data_time.val, bt=batch_time.val,
                      total=None, eta=None, loss=loss_str, top1=top1_str, top5=top5.avg))
        """
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(val_loader):
        ...
        bar.suffix = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} ' \
            ...
            .format(batch=batch_idx + 1, size=len(val_loader), data=data_time.avg, bt=batch_time.avg,
...
class MixBatchNorm2d(nn.BatchNorm2d):
    ...
    that of the inputs of the MixBatchNorm2d should be [2*N, 3, 224, 224].
       If you set batch_type as 'mix', this network will using one batchnorm (main bn) to calculate the features
    corresponding to[:N, 3, 224, 224], while using another batch normalization (auxiliary bn) for the features of
    [N:, 3, 224, 224].
       ...
       If you set batch_type as 'clean', the features are calculated using main bn; if you set it as 'adv',
    the features are calculated using auxiliary bn.
       Usually, we use to_clean_status, to_adv_status, and to_mix_status to set the batch_type recursively.
    It should be noticed that the batch_type should be set as 'adv' while attacking.
    """
