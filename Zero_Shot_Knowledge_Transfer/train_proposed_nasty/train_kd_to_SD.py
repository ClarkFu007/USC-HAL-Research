...
parser.add_argument('--save_path', default='KD_SD/resnet50_self18_2', type=str)
parser.add_argument('--teacher_resume', default=None, type=str,
                    help='If you specify the teacher resume here, '
                         'we will use it instead of parameters from json file')
...
def loss_fn_kd(outputs, labels, teacher_outputs, epoch, params):
    ...
    # Souvik: do only CE loss before the kd_delay
    else:
...
def train_and_eval_kd(model, t_model, optim, loss_fn, train_loader, dev_loader, params):
    ...
    for epoch in range(params.num_epochs):
        ...
        #Souvik: As we are validating on the self_distill student models so, is_self=True, else its False
