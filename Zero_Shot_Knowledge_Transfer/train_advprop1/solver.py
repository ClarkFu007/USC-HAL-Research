...
class MixBatchNorm2d(nn.BatchNorm2d):
    ...
    that of the inputs of the MixBatchNorm2d should be [2*N, 3, 224, 224].
       If you set batch_type as 'mix', this network will using one batchnorm (main bn)
    to calculate the features corresponding to[:N, 3, 224, 224], while using another
    batch normalization (auxiliary bn) for the features of [N:, 3, 224, 224].
    ...
    the features are calculated using auxiliary bn. Usually, we use to_clean_status, to_adv_status, and
    to_mix_status to set the batch_type recursively. It should be noticed that the batch_type should be set
    as 'adv' while attacking.
    """
...
class ZeroShotKTSolver(object):
    ...
    def __init__(self, args):  # The constructor in Python
        ...
        attack_iter, attack_epsilon, attack_step_size = 0, 5.0, 1.0
        if attack_iter == 0:
        ...
        teacher_arch = "wide_resnet50_2"
        self.teacher = net_rectified.__dict__[teacher_arch](num_classes=10, norm_layer=norm_layer)
        ...
        self.mixbn = True  # Whether to make the student have mixbn or not
        if self.mixbn:
            to_merge = {}
        ...
        student_arch = "wide_resnet18_2"
        self.student = net_rectified.__dict__[student_arch](num_classes=10, norm_layer=norm_layer)
        self.student.set_attacker(attacker)
        self.student.set_mixbn(False)
        ...
        self.optimizer_student = optim.SGD(self.student.parameters(), lr=args.student_learning_rate,
    ...
    def run(self):
        ...
        while self.n_pseudo_batches < self.args.total_n_pseudo_batches:
            ...
            if idx_pseudo % self.n_repeat_batch < self.args.n_generator_iter:
                ...
                if self.n_pseudo_batches % 2 == 0:
                    self.student.apply(to_clean_status)
                    self.teacher.apply(to_clean_status)
                    student_logits, *student_activations = self.student(x_pseudo)
                    teacher_logits, *teacher_activations = self.teacher(x_pseudo)
                else:
                    self.student.apply(to_adv_status)
                    self.teacher.apply(to_adv_status)
                    student_logits, *student_activations = self.student(x_pseudo)
                    teacher_logits, *teacher_activations = self.teacher(x_pseudo)
