...
class Logger(object):
    ...
    def image_summary(self, tag, images, step):
        ...
        if len(images.shape) == 4:
            ...
            # summary = tf.Summary(value=[tf.Summary.Value(tag=tag, simple_value=value)])
            with self.writer.as_default():
                # print("images.reshape([1, images.shape[1], images.shape[2], 3]).shape",
                      # images.reshape([1, images.shape[1], images.shape[2], 3]).shape)
                tf.summary.image(tag, images.reshape([1, images.shape[1], images.shape[2], 3]), step)
                self.writer.flush()
            """
            img_summary = tf.Summary.Image(encoded_image_string=s.getvalue(),
