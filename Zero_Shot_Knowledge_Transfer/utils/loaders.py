...
class LearnableLoader(nn.Module):
    def __init__(self, args, n_repeat_batch):
        ...
        """
            "self._" is inside the instance of the class. The result is that it will be visible to all other methods. 
        The variable remains alive until the instance of the class is destroyed.
        """
        self._running_repeat_batch_idx = 0
        self.z = torch.randn((args.batch_size, args.z_dim)).to(device=args.device)

    def __next__(self):  # For iteration
        if self._running_repeat_batch_idx == self.n_repeat_batch:
            self.z = torch.randn((self.batch_size, self.z_dim)).to(device=self.device)
            # print("running_repeat_batch_idx", self._running_repeat_batch_idx)
            self._running_repeat_batch_idx = 0
    ...
    def samples(self, n, grid=True):
        ...
        """
            ...
            Disabling gradient calculation is useful for inference, when you are sure that you 
        will not call Tensor.backward(). It will reduce memory consumption for computations that 
