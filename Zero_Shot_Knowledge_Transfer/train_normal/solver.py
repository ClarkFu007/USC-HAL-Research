...
class ZeroShotKTSolver(object):
    ...
    def __init__(self, args):  # The constructor in Python
        ...
        from resnet_teacher import ResNet34, ResNet18
        self.teacher = ResNet34(num_class=10)
        self.teacher = self.teacher.to(args.device)
        ...
        self.student = ResNet18(num_class=10)
        self.student = self.student.to(args.device)
        self.student.train()
        """
        
        from models.wresnet import WideResNet
        self.teacher = WideResNet(depth=50, num_classes=10, widen_factor=2, dropRate=0.0)
        self.teacher = self.teacher.to(args.device)
        model_path = args.pretrained_models_path
        print('Loading Model from {}'.format(model_path))
        checkpoint = torch.load(model_path)
    ...
    def run(self):
        ...
        while self.n_pseudo_batches < self.args.total_n_pseudo_batches:
            ...
            if idx_pseudo % self.n_repeat_batch < self.args.n_generator_iter:
                ...
                # print("teacher_logits.shape", teacher_logits.shape)  # torch.Size([128, 10])
                # print("teacher_logits", teacher_logits)
                # print("*teacher_activations", type(*teacher_activations))

                # Maximize D_kl for the generator loss without an attention term
                ...
                if self.n_pseudo_batches % 100 == 0 or self.n_pseudo_batches <= 100:
                    ...
                    if target_i != -1:
                        vutils.save_image(x_pseudo[target_i].data.clone(),
                                          '/home/yaof/zero_shot/saved_images/normal/output_{}.png'
