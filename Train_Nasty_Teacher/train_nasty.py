...
parser.add_argument('--save_path', default='/home/yaof/undistillable/', type=str)
...
def train_epoch_kd_adv(model, model_ad, optim, data_loader, epoch, params):
    ...
    with tqdm(total=len(data_loader)) as t:  # Use tqdm for progress bar
        for i, (train_batch, labels_batch) in enumerate(data_loader):
            ...
            output_tch, _, _, _, _ = model(train_batch)  # logit without SoftMax
            ...
            with torch.no_grad():
                output_stu, _, _, _, _ = model_ad(train_batch)  # logit without SoftMax
...
def evaluate(model, loss_fn, data_loader, params):
    ...
    with torch.no_grad():
        ...
        for data_batch, labels_batch in data_loader:
            ...
            output_batch, _, _, _, _ = model(data_batch)
    ...
    for epoch in range(params.num_epochs):
        ...
        if val_acc >= best_val_acc:
            ...
            torch.save({
                ...
                save_name)
            """
            save_name = os.path.join(args.save_path, 'best_model')
